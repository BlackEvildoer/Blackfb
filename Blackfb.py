...
# coding=utf-8
# Coded By BlackMiror
# My Telegram ( https://t.me/BESTxHACKER)
...
def yayanxd():
    os.system('clear')
    print ('\x1b[1;91m[*] Bo Henany Token Bro Barnamay GetAcess TokenDabzena.\n')
    print('\x1b[1;95m [*] Auther : BLACKMIROR\n')
    print('\x1b[1;96m [*] JOIN CHANEL : @BESTxHACKER')
    ...
    try:
        ...
        raw_input(' %s*%s Enter Bka'%(O,N));wuhan(kontol)
        os.system('xdg-open https://t.me/BESTxHACKER')
        moch_yayan()
    except KeyError:
...
def moch_yayan():
    ...
    print ' [%s7%s]. \x1b[1;95mLihat hasil crack'%(O,N);time.sleep(0.03)
    print ' [%s8%s]. \x1b[1;92mGORENI IP XOT'%(O,N);time.sleep(0.03)
...
def teman(kontol):
    ...
    try:
        mmk = raw_input('\n %s[%s?%s] Nawe File  : '%(N,O,N))
        asw = raw_input(' %s[%s?%s] CHNAD ID BET   : '%(N,O,N))
        ...
        xxx.close()
        jalan('\n\n %s[%s✓%s] Extarack File Saved'%(N,H,N))
...
def publik(kontol):
    ...
    try:
        csy = raw_input('\n %s[%s?%s] ID Publice  : '%(N,O,N))
        ahh = raw_input(' %s[%s?%s] Nawe File  : '%(N,O,N))
        ...
        xxx.close()
        jalan('\n\n %s[%s✓%s] Extrack ID File Saved'%(N,H,N))
...
def followers(kontol):
    ...
    try:
        ...
        mmk = raw_input(' %s[%s?%s] Nawe File  : '%(N,O,N))
        ...
        xxx.close()
        jalan('\n\n %s[%s✓%s] Extarack File Saved total followers'%(N,H,N))
...
def postingan(kontol):
    ...
    try:
        csy = raw_input('\n %s[%s?%s] ID POST : '%(N,O,N))
        ppk = raw_input(' %s[%s?%s] NAWE FILE  : '%(N,O,N))
        asw = raw_input(' %s[%s?%s] CHAND ID BET  : '%(N,O,N))
...
def info_tools():
    ...
    print '\n %s[%s>%s] Auther       : BLACK MIROR'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Telegram     : BESTxHACKER'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Github       : https://github.com/fardinHack'%(N,H,N);time.sleep(0.07)
...
def seting_yntkts():
    ...
    if ytbjts == '':
        ...
        try:
            ...
            user_agent = '%s-'%(M)
        print '\n %s[%s+%s] IP BNUSA : %s%s'%(N,O,N,H,user_agent)
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            raw_input('\n  %s[ %skembali%s ]'%(N,O,N));moch_yayan()
        ___yayanganteng___ = raw_input(' [%s?%s] Xot Pass Da Aney.? [Y/t]: '%(O,N))
        if ___yayanganteng___ in ('Y', 'y'):
            print '\n %s[%s!%s] gunakan , (koma) Nmuna : sandi123,sandi12345,dll. setiap kata minimal 6 karakter atau lebih'%(N,M,N)
            while True:
